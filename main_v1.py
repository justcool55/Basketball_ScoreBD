...
class ScoreboardApp(App):
    def build(self):
        ...
        # ===== 상단 컨트롤 영역 =====
        top = BoxLayout(orientation='horizontal', size_hint_y=0.12, spacing=8)
        ...
        timer_ctrl = BoxLayout(orientation='horizontal', size_hint_x=0.5, spacing=6)
        ...
        # --- build() 안, 컨트롤 바 버튼들 추가 부분 ---
        self.switch_btn = Button(text='SWITCH', background_color=(0.5,1,0.5,1))
        self.switch_btn.bind(on_press=lambda *_: self.swap_teams())
        #top.add_widget(switch_btn)


        for b in (self.start_btn, self.pause_btn, self.restart_btn, self.reset_btn, self.buzzer_btn, self.switch_btn):
    ...
    def load_default_buzzer(self):
        for path in ['D:\Python_work_Space\Practice\BasketBall_ScoreBoard\Buzzer.mp3']:
            if os.path.exists(path):
    ...
    def _change_quarter(self, step):
        self.q_index = (self.q_index + step) % len(self.qs)
        self.q_label.text = self.qs[self.q_index]
        # 쿼터가 바뀔 때마다 자동 코트 교대
        self.swap_teams()
    ...
    def swap_teams(self):
        ...
        self.team1_score_lbl.text = str(self.team1_score)
        self.team2_score = str(self.team2_score)

        # 이미지 교환
        left_src = self.white_img.source
        right_src = self.black_img.source
        self.white_img.source = right_src
        self.black_img.source = left_src
